...
| 8 | aviso de depreciação do LangGraph | **REPRODUZIDO na 2ª tentativa** — ver adendo |
...
**#8 — o warning do LangGraph. REPRODUZIDO no fim da sessão, e a primeira leitura estava errada.**
Ele aparece no log do servidor quando a interface roda uma consulta ao vivo, não só ao ler run salvo:

    Deserializing unregistered type src.state.PlanoDeBusca from checkpoint. This will be blocked
    in a future version. Set LANGGRAPH_STRICT_MSGPACK=true to block now, or add to
    allowed_msgpack_modules to allow explicitly: [('src.state', 'PlanoDeBusca')]

Idem `StartupRef` e `AnaliseStartup`. **O conserto está nomeado dentro da própria mensagem** —
registrar os três tipos em `allowed_msgpack_modules` — e não entra hoje porque é compatibilidade
futura, não falha de hoje, e mexer na serialização do checkpointer na véspera troca risco alto por
zero ganho. **Fica como P-28.**

O erro de método vale mais que o achado: eu o dei por não reproduzido depois de UMA tentativa, num
caminho (`--thread`) que não era o que a auditoria descreveu. **"Não reproduzi" não é "não existe"**,
e a distinção só apareceu porque a sessão continuou rodando o sistema.
...
| **P-28** | **LangGraph vai bloquear a desserialização dos tipos de `src.state` do checkpoint** | aberta por **D-121**. `PlanoDeBusca`, `StartupRef` e `AnaliseStartup` são desserializados como tipos não registrados, e o aviso diz *"this will be blocked in a future version"*. **O conserto está na própria mensagem:** registrar os três em `allowed_msgpack_modules`. Não entrou em 08/09 porque é compatibilidade futura e não falha de hoje — mexer na serialização do checkpointer na véspera é risco alto por ganho zero. Reproduzível: subir `python -m src.web`, rodar uma consulta, olhar o log do servidor. Depois da entrega |

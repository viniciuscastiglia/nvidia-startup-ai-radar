...
| base | **30 startups, 93 documentos, 93/93 URLs** (D-090) · `ano_fundacao` literal em **18** de 30, `estagio` em **11** e `localizacao` em **9** (medido em 04/09; a linha anterior dizia 12 e 5) — baixo **de propósito**: só entra o que está literalmente no documento |
...
| **Projeto que não executa e cujo vídeo não demonstra funcionamento real** | o TAPI exige a demo **pela interface web** — logo a interface **não é os 5 pontos do critério 4, é a porta do eliminatório** | **FECHADO em 06/09 (D-107):** a interface existe e rodou o grafo ponta a ponta com `RERANK_PROVEDOR=cohere`, 2m18 para 3 empresas. O que sobra é gravar |
...
| ~~**P-06 + interface**~~ | **FEITA em 06/09 (D-107).** FastAPI + uvicorn, front à mão, run ao vivo por SSE mostrando os 4 nós do pai e os 6 do subgrafo por empresa. As 4 etapas fechadas — consultar → ver → recomendações com evidência clicável → exportar o briefing **byte a byte** — mais a **vitrine do passo 7**, que mostra as duas ordens da mesma recuperação. A camada não tem regra de negócio: a ordem das empresas e o rótulo do quadrante foram **extraídos** de `briefing.py` para que a tela não discorde do `.txt` que ela exporta. `pytest` 93 → **102** | ~~04–05/09~~ | ✅ |
...
| **P-11** *(metade FEITA em 04/09, D-100)* | `confianca` é **0/6 constante** — campo do briefing com zero informação. ~~**E pior do que se sabia (D-089):** o briefing imprimia *"— ver D-059 …"*, ponteiro interno de decisão dentro do entregável~~ — **essa metade fechou** | **05/09** | **O ponteiro `D-059` SAIU do texto do usuário em 04/09 (D-100).** O que sobra é o `min()`, e `medir_confianca.py` mostrou que ele não se conserta por aqui: o braço da hipótese de D-059 (evidência mais larga) dá **2, idêntico**; o que move é a recência, e `data_publicacao` falta em **86 dos 93 documentos**. Consertar o campo é consertar a coleta — que é P-25, depois da entrega |
| ~~**P-21**~~ | **A RÉGUA EXISTE desde 05/09 (D-105), e o motor REPROVA nela.** `--regras-tapi` mede as 7 regras de `contexto/01-tapi.md:143` sobre as 30: **motor 13/34 = 38% contra 15/34 = 44% da linha trivial**. A razão do ACEITAR anterior (*"só 1-2 das 7 regras têm startup"*) era de quando a base tinha 8 empresas: **são 7 de 7**. O corte que explica: onde o conjunto esperado contém uma das 3 dominantes, o trivial faz 15/15 por construção e o motor 7/15; onde só o domínio resolve, o **trivial faz 0/19 e o motor 6/19** — ele é o único que recupera Isaac, Omniverse e RAPIDS. A causa está na distribuição: **`NVIDIA NeMo` e `AI Enterprise` são 46% das 89 recomendações**. **Consertar o motor é redesenho e não cabe antes do vídeo** — o que cabe, e agora existe, é o número e a causa | ~~05/09~~ | ✅ régua |
| ~~**`justificativa_negocio`** (P-22)~~ | **FEITO em 05/09 (D-104), e o defeito era OUTRO.** Não era falta de texto para 11 tecnologias: era **índice**. `NEGOCIO` era chaveado só pela tecnologia, e a dor que puxou a citação vive em `citacao.dor_origem` desde D-043 — o código tinha a informação e jogava fora. Na tela, com o rerank ligado: NIM sob `latencia` dizendo *"reduz o custo por token"*. Medido no briefing inteiro: **2 de 15** recomendações coerentes com a dor declarada. Passa a `(tecnologia, dor)` mais **8 frases por dor** de `contexto/03` §4 — que cobrem as 128 células e matam o fallback. **15 de 15** no mesmo run. E o conserto tropeçou no teto de 150 do briefing: sete das oito frases estouravam e teriam chegado cortadas (D-100 por outra porta) | ~~05/09~~ | ✅ |
...
| ~~**P-14**~~ | **FEITA em 06/09 (D-112), e o achado estava numa linha:** `estrategia_analise` era uma **string CONSTANTE**, então "o subgrafo passa a lê-la" teria satisfeito a letra sem mudar nada. O planner passa a derivar `dores_prioritarias` da consulta e `nvidia_rag` ordena as dores por ela — e como `recommendation` corta em 3, **isso decide qual tecnologia a empresa recebe**. Os outros três ganharam leitor de exibição. **Régua idêntica** (`dores_prioritarias=[]` é a identidade, e é o único ramo que a régua exercita) **e não inerte**: 34% dos 131 pares empresa × dor trocam a tecnologia | ~~06/09~~ | ✅ |
| ~~**README**~~ | **FEITO em 06/09.** Os 4 buracos do clone limpo fechados: `environment.yml` (o repositório não sabia criar o próprio ambiente), `createdb` escrito, a `DATABASE_URL` do Docker escrita, `COHERE_REQ_POR_MIN`. Mais a tabela de stack sem os quatro *"a definir"*, a estrutura com `src/`, e **o ponteiro para `projeto/decisoes.md`** — o item 7 de §3.3. Os números do README saíram de execução: 112 decisões, 30 fixtures, 128 testes, 377 chunks | ~~06/09~~ | ✅ |
| ~~**Diagramas `.mmd`**~~ | **JÁ ESTAVAM EM DIA — verificado em 05/09.** A linha dizia *"são de 23/08 e nunca foram regenerados"*; `subgrafo-analise.mmd` foi regenerado em 04/09 junto com D-099, e rodar `diagramas.py` hoje produz **zero diff**. O que sobra do item é P-14, que é outra linha | ~~06/09~~ | ✅ |
| ~~**Docstrings mentem**~~ | **FEITO em 06/09.** Os três textos passam a dizer **por que** a escolha é medida: `query_planner` (latência no primeiro nó, zero LLM no grafo, vocabulário fechado), `recommendation` (D-086 e D-104 mediram os dois campos, e a opção (b) segue registrada) e `briefing` (todo texto é composição de campos que já carregam evidência) | ~~06/09~~ | ✅ |
| ~~**As 16 fontes do RAG não estão cacheadas**~~ | **FEITO em 06/09 (D-110).** O corpus entrou no git; `ingerir_nvidia.py` **não toca a rede** sem `--refetch`, que re-baixa e **imprime o que mudou**. Verificado ponta a ponta: ingestão real do cache para o banco, **175 + 202**, e `avaliar_rag --validar` em **24/24**. **O teste achou um defeito que nenhuma saída mostrava:** `write_text`/`read_text` divergem em CRLF, e 7 das 16 fontes vêm com CRLF | ~~06/09~~ | ✅ |
| ~~**`.env.example` incompleto**~~ | **FEITO em 03/09.** `COHERE_REQ_POR_MIN` documentado, e entrou junto o aviso de D-093: a cota mensal acabou, e `RERANK_PROVEDOR=nenhum` é o default de desenvolvimento — com o número que o justifica (`pytest` 81/81 em 6,5 s contra 150-460 s) | ~~06/09~~ | ✅ |
...
|---|---|
| **P-12** — `classe` 3/7 | D-060 calculou o **teto: 4/7**, porque 7 das 8 fixtures têm profundidade técnica zero. O gargalo é vocabulário, não a regra. E a base nova entra **como dado, sem gabarito** (D-062), então a régua não se move nem com 30 empresas — **confirmado por medição em 03/09: com 16 fixtures, `classe 3/7` e precisão 49% reproduzem exatamente** (D-090). Mexer no limiar seria calibrar contra o gabarito. **E EM 05/09 A SEGUNDA HIPÓTESE DE D-060 TAMBÉM CAIU (D-106):** a lista alternativa de `PROFUNDOS` — 29 termos derivados da própria rubrica e do documento da stack, commitada antes de medir — dá **`classe 4/7`**, empata com o braço em degraus e com o trivial. Ela acrescenta 5 marcadores à **única** empresa que já passava e **zero às outras 29**; nove dos 17 termos novos não ocorrem em nenhuma das 30; e `avaliaç`, que a rubrica nomeia literalmente, ocorre em 13 empresas e em **12 é outro sentido**. Com a hipótese 1 refutada em D-095, **as duas de D-060 estão mortas, e a conclusão é sobre o mundo:** as sete fixtures que o classificador erra não têm vocabulário de profundidade nenhum. **Talvez ele esteja certo** — se uma healthtech não escreve uma linha sobre avaliação de modelo ou guardrails, ela consome API, que é a tese do TAPI. **O que mudou é a visibilidade:** rodando o grafo, a **Core AI** — cujo produto É modelo de crédito com IA — sai `non-AI`. O defeito continua aceito; o custo dele agora é uma cena, não uma linha de tabela |
| ~~**P-24**~~ — `non-AI` é default de detecção falha | **FECHADA em 04/09 (D-101), e a razão do ACEITAR estava desatualizada.** Ela dizia *"consertar exige gabarito"* — o que é verdade para os DETECTORES e para a quarta classe, e falso para o conserto que entrou: `sinal_verificado` não introduz um único grau de liberdade, e `varrer_classes.py --custo-desenhos` mediu **`3/7 → 3/7`** ANTES de implementar. **Fica aberta só a parte 2 do critério:** o ruído das 9 que passaram a entrar no funil, contra as 7 regras do TAPI, em **05/09** |
...
| **P-23** — `elegibilidade()` só lê os trechos de evidência | **novo, D-091.** A Liqi diz *"oferecer criptomoedas, stablecoins e tokens"* no site, `criptomoeda` **já estava na lista de exclusão**, e ela passava — a frase não caiu em nenhum trecho citado pelo Extractor. O filtro do Inception, que é o **Diferencial declarado**, tem a cobertura do casador de dores. **A correção óbvia — varrer `conteudo_texto` — QUEBRA O VETO DE TERCEIRO POR CONSTRUÇÃO, e este é o argumento medido em 04/09:** `_fala_de_terceiro` exige que **toda** ocorrência do termo caia em frase com marcador, então cada caractere a mais é outra chance de o `all()` falhar. Medido: as recusas vão de **7 para 13** em 30, e ao menos 4 das 6 novas são falso positivo claro. **E o lado silencioso apareceu junto:** a varredura ampla recusa a Produzindo Certo, que *"oferece serviços de consultoria"* — a própria empresa; hoje ela passa por sorte, não por desenho. Redesenho, não ajuste. **Fica ACEITO e escrito — e a frase da defesa é essa, com os dois lados** |
...
| ~~**Interface escorregar e não haver demo**~~ | **NÃO se materializou: feita em 06/09** (D-107). E ela trouxe a própria rede: um run commitado em `data/runs/exemplo-*.json`, que a tela abre sem chamar API nenhuma — se o fornecedor morrer na hora de gravar, a demo continua existindo |

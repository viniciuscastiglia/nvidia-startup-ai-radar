...
| **P-24** — `non-AI` é default de detecção falha | **novo, D-095.** Medido na base de 30: `AI-native` **1**, `AI-enabled` 20, `non-AI` **9** — e `non-AI` sai de `pontos == 0`, ou seja, de três detectores do Extractor voltarem vazios. A **Core AI** (5 dores de IA, "AI" no nome) e a **Visio.AI** (site: *"AI-Native Operating System"*) saem `non-AI` → `fora-do-funil` → **zero recomendações**. **É o oposto do princípio que `Elegibilidade` já aplica** (`motivos_exclusao` vs `requisitos_nao_verificados`; regra 4 do Evidence Validator). **Fica ACEITO porque consertar exige gabarito** — as 22 novas entram sem ele (D-062), e calibrar detectores até a Core AI sair certa é calibrar contra o próprio julgamento. **A frase da defesa muda:** não "acerta 3/7", e sim "não distingue *sem sinal* de *não achei sinal*, e isso tira 9 de 30 do funil" |

...
> **ATUALIZADO EM 28/08/2026 — a stack de recuperação morreu TRÊS vezes em três meses.** Este
> catálogo aposenta modelos com **HTTP 410/404** em cadência de meses, e isso é um fato sobre o
> fornecedor, não um acidente:
>
...
> | **27/08/2026** | `meta/llama-3.1-8b-instruct` (410) e `nvidia/rerank-qa-mistral-4b` (404) | D-012, D-046 |
>
> **Do NeMo Retriever só o EMBEDDING sobreviveu.** Não há reranker no catálogo da NVIDIA: 9
> sondagens de path × modelo em 28/08, todas 404/410, e zero modelos com `rank` no nome entre os
> 83 listados. O passo 7 do projeto é **Cohere `rerank-v3.5`** desde 28/08 (D-068).
>
> E **estar em `GET /v1/models` não é estar vivo** — 9 dos 10 candidatos mortos estavam listados
> (D-070). Antes de qualquer decisão que dependa da tabela abaixo, rode
> `python scripts/sondar_catalogo.py --structured --rerank`.

| Papel | Modelo atual | Endpoint | Medido em 28/08 |
|---|---|---|---|
| Chat | **`nvidia/nemotron-3-nano-30b-a3b`** | `https://integrate.api.nvidia.com/v1/chat/completions` | ~650 ms · **1 utilizável de 10 candidatos sondados** (D-067) |
| Embedding | **`nvidia/llama-nemotron-embed-vl-1b-v2`** | `https://integrate.api.nvidia.com/v1/embeddings` | 615 ms · 2048 dims nativas, **aceita `dimensions`** 1024 e 2048 (Matryoshka) |
| Embedding | `nvidia/nemotron-3-embed-1b` | idem | 2048 dims, **não trunca** (HTTP 400 em `dimensions`) |
| Reranking | ~~`nvidia/rerank-qa-mistral-4b`~~ | ~~`.../v1/retrieval/nvidia/reranking`~~ | **MORTO em 27/08 (404).** Sem substituto no catálogo |
...
1. **A URL de reranking mudou de forma antes de sumir.** A primeira embutia o nome do modelo no
   path; a segunda era genérica e o modelo ia no corpo. `src/config.py` guarda a URL completa
   justamente porque a forma dela não é estável — e desde D-068 guarda também o **provedor**, que
   era a variável que faltava: trocar o modelo só resolve se existir outro modelo.
...
> **Por que isso importa para o projeto — e a frase que estava aqui tinha um fato falso.** Ela
> dizia que o NeMo Retriever *"substitui o Cohere Rerank (pago)"*. **O Cohere tem trial key
> gratuita** cobrindo Rerank (D-065), e desde 28/08 é ele que roda o passo 7 (D-068).
>
> O que sobra de pé, e é mais forte: o **embedding** resolve documentos em português com
> crosslingual real, e continua rodando na stack NVIDIA. **O Diferencial do projeto não é "usar a
> stack NVIDIA"** — é ter medido uma propriedade do fornecedor que ninguém mede: três EOLs em três
> meses, com data, método e instrumento versionado (D-070).
